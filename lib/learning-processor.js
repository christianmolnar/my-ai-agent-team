...
class LearningProcessor {
   ...
   */
  static async processPendingLearnings() {
    const result = {
      processed: 0,
      created: [],
      updated: [],
   ...
   */
  static async materializeLearning(learningEntry, analysisData) {
    const createdFiles = [];
   ...
   * Ensure CNS directory structure exists
   */
  static async ensureCNSStructure(cnsPath) {
    const dirs = [
      join(cnsPath, 'brain'),
   ...
   */
  static getAgentFolderName(agentType) {
    const mapping = {
   ...
   */
  static getMemoryFileName(content) {
   ...
   */
  static generateCNSContent(update, analysisData, learningEntry) {
...
${analysisData.reflexUpdates ? 
  analysisData.reflexUpdates.map(reflex => `- ${reflex.trigger}: ${reflex.response}`).join('\n') : 
...
`;
      ...
      case 'trust_management.json':
        return {
          ...
          trustEvaluationCriteria: {
            ...
            informationAccuracy: {
              factChecking: true,
              patterns: analysisData.memoryEnhancements?.filter(m => m.type === 'semantic') || [],
   ...
   */
  static generateMemoryContent(memory, analysisData, learningEntry) {
...
${analysisData.newCapabilities.map(cap => `- ${cap}`).join('\n')}` : ''}
...
`;
   ...
   */
  static generateReflexContent(reflexUpdates, analysisData, learningEntry) {
...
// Export the class
module.exports = { LearningProcessor };
