...
const EnhancedFileSystemManager_1 = require("../lib/EnhancedFileSystemManager");
class ResearcherAgent {
    constructor() {
        ...
        this.aiClient = new universal_ai_client_1.UniversalAIClient();
        this.fileManager = new EnhancedFileSystemManager_1.EnhancedFileSystemManager();
    }
    /**
     ...
     */
    async generateGeneralResponse(request, conversationHistory) {
...
Generate the complete, full-length content now - don't provide instructions on how to create it.`
     ...
     */
    async attemptToSaveAsDeliverable(content, request) {
        try {
            const filename = await this.generateFilename(request);
            const saveResult = await this.fileManager.saveResearchPaper(filename, content, `Research response: ${request}`, this.id);
            return {
     ...
     */
    async generateFilename(request) {
        try {
            const messages = [
                {
                    role: 'user',
...
All agents are operational and ready for task execution.`;
     ...
     */
    async generateActualPaperContent(request) {
        const messages = [
            {
                role: 'user',
...
Please generate the complete paper content in Markdown format, ensuring it's comprehensive, factual, and professionally written.`
     ...
     */
    getFallbackPaperContent(request) {
