...
exports.MusicCoachAgent = void 0;
/**
 ...
 */
class MusicCoachAgent {
    constructor() {
        this.id = 'music-coach';
        this.name = 'Music Coach Agent';
        this.description = 'Music theory education and instruction specialist';
        this.abilities = [
...
5. Next steps for advancement`;
     ...
     */
    async analyzeChordProgression(payload) {
...
Provide a detailed but accessible analysis suitable for music education.`;
    ...
    async createPracticeSession(payload) {
        const { level = 'beginner', instrument = 'piano', duration = 30, // minutes
        focus_area = 'general' } = payload;
...
Create a detailed, structured practice session that's engaging and educational.`;
     ...
     */
    async generateTheoryLesson(payload) {
...
Create an engaging theory lesson that makes ${topic} understandable and applicable.`;
     ...
     */
    async assessSkillLevel(payload) {
...
Once comfortable with these basic triads, we'll explore 7th chords and more complex progressions.`;
    ...
    generateChordExercises(key, level) {
        return [
            {
    ...
    generateRomanNumerals(chords, key) {
    ...
    suggestProgressionVariations(chords, key) {
        return [
            {
    ...
    generateMockChordLesson(instrument, level, key, style) {
        return {
            lesson_type: 'chord_instruction',
            instrument,
            level,
    ...
    generateMockProgressionAnalysis(chords, key) {
        return {
            analysis_type: 'chord_progression',
            original_progression: chords,
            key,
    ...
    generatePracticeExercises(level, instrument) {
        return [
            {
                name: 'Scale Practice',
                duration: '5 minutes',
    ...
    planNextSession(level, focus_area) {
        return {
            suggested_date: 'In 2-3 days',
            focus: `Continue with ${focus_area} and introduce new concepts`
        };
    }
    generateMockPracticeSession(level, instrument, duration) {
        return {
            session_type: 'practice_session',
            level,
            instrument,
    ...
    generateTheoryExercises(topic, level) {
        return [{
    ...
    generateQuizQuestions(topic, level) {
        return [{
    ...
    generateMockTheoryLesson(topic, level) {
        return {
            lesson_type: 'theory',
            topic,
            level,
     ...
     */
    async executeMusicCoachingTask(payload) {
        try {
     ...
     */
    async performMusicCoaching(userRequest, payload) {
        try {
...
*Analysis: ${analysis.substring(0, 200)}...*`;
     ...
     */
    async provideFallbackResponse(userRequest, payload) {
...
Let me know how I can best support your musical journey!`;
     ...
     */
    createMusicCoachingPlan(request, payload) {
...
exports.MusicCoachAgent = MusicCoachAgent;
