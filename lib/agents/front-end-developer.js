...
const FileSystemManager_1 = require("../lib/FileSystemManager");
...
This solution delivers a high-quality, modern frontend experience with excellent performance and user experience.`;
    ...
    async generateExecutableCode(request) {
        const messages = [
            {
                role: 'user',
...
Provide only the complete HTML code that can be saved and run directly.`;
        try {
            ...
            try {
                const saveResult = await FileSystemManager_1.fileSystemManager.extractAndSaveHtml(response.content, projectName);
