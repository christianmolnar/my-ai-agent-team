...
const universal_ai_client_1 = require("../lib/universal-ai-client");
/**
...
class MasterOrchestratorAgent {
    ...
    **SELECTED AGENTS:**
    ...
    Use the exact agent IDs from the available agents list above. Only select agents that are actually needed for the task.`;
        try {
            const response = await universal_ai_client_1.universalAIClient.generateResponse(this.id, messages, systemPrompt);
     ...
     */
    async orchestrateTaskWithReview(payload) {
        try {
            ...
            const revisedResults = await this.integrateReviewFeedback(plan, payload, initialResults, reviewResult);
            return {
                status: 'completed',
                executedAgents: [...plan.agents, 'reviewer'],
                completedDeliverables: payload.deliverables || [],
     ...
     */
    shouldTriggerReview(payload, plan) {
        ...
        const hasFileDeliverables = payload.deliverables?.some((d) => d.includes('file') || d.includes('document') || d.includes('report'));
        return hasReviewTrigger || isComplexTask || hasFileDeliverables;
    }
    /**
     * Conduct review using the Reviewer Agent
     */
    async conductReview(reviewRequest) {
        try {
            console.log('🔍 Master Orchestrator: Initiating review process');
            // Get reviewer agent instance
            const reviewerAgent = await agent_registry_1.AgentRegistry.getAgentInstance('reviewer');
            if (!reviewerAgent) {
     ...
     */
    async integrateReviewFeedback(plan, payload, originalResults, reviewResult) {
        try {
...
**Output:** Provide the revised version that integrates the review feedback while maintaining the original structure and core content.`;
            ...
            const agentInstance = await agent_registry_1.AgentRegistry.getAgentInstance(primaryAgent);
            if (!agentInstance) {
     ...
     */
    async buildPlanningPrompt(payload) {
        // Dynamically get all available agents from the registry
        const availableAgents = await agent_registry_1.AgentRegistry.getAvailableAgents();
        const agentDescriptions = availableAgents
            .filter(agent => agent.id !== this.id && agent.id !== 'master-orchestrator') // Exclude self
            .map(agent => {
            const displayName = agent_registry_1.AgentRegistry.getAgentDisplayName(agent);
...
Please provide a structured plan that leverages the appropriate agents for this specific task.`;
     ...
     */
    async getAvailableAgentCount() {
        const agents = await agent_registry_1.AgentRegistry.getAvailableAgents();
     ...
     */
    async getAvailableAgentsDescription() {
        const agents = await agent_registry_1.AgentRegistry.getAvailableAgents();
        const filteredAgents = agents.filter(agent => agent.id !== this.id && agent.id !== 'master-orchestrator');
        return filteredAgents.map(agent => `- ${agent.id}: ${agent.name} - ${agent.description || 'Specialized agent'}`).join('\n');
    }
    /**
     * Parse planning response from Claude
     */
    async parsePlanningResponse(response, payload) {
    ...
    async executePlan(plan, payload) {
        ...
        const sessionId = await interaction_logger_1.interactionLogger.startChatSession(payload.userId || 'unknown', payload.userRequest || 'Agent orchestration request', `Orchestrating task with ${plan.agents.length} agents`);
        const executionResults = [];
        // Log the orchestrator's initial task
        const orchestratorInteractionId = await interaction_logger_1.interactionLogger.logAgentInteraction(sessionId, this.id, this.name, `Orchestrate multi-agent task: ${payload.userRequest || 'Unknown task'}`, JSON.stringify(payload), 'management', 'high', 'complex', 'user');
        // Execute each agent in the plan
        for (const agentId of plan.agents) {
            try {
                // Log agent interaction
                const agentInteractionId = await interaction_logger_1.interactionLogger.logAgentInteraction(sessionId, agentId, this.getAgentDisplayName(agentId), this.getAgentTask(agentId, payload), payload.userRequest || 'Orchestrated task', 'specialist', 'medium', 'moderate', this.id);
    ...
    async executeRealAgent(agentId, payload) {
        try {
            ...
            const agentInstance = await agent_registry_1.AgentRegistry.getAgentInstance(agentId);
            if (!agentInstance) {
                const availableAgents = await agent_registry_1.AgentRegistry.getAvailableAgents();
    ...
    getAgentDisplayName(agentId) {
        return agent_registry_1.AgentRegistry.getAgentDisplayName(agentId);
    }
    /**
    ...
    async getAgentCapabilities() {
        ...
        const targetAgents = availableAgents.filter(agent => agent.id !== this.id && agent.id !== 'master-orchestrator' && agent.id !== 'master-orchestrator-agent');
        ...
        for (const agent of targetAgents) {
            try {
                ...
                // Get the actual agent instance
                const agentInstance = await agent_registry_1.AgentRegistry.getAgentInstance(agent.id);
                if (!agentInstance) {
                ...
                if (result.success && result.result) {
                    ...
                    capabilities.push(`**${agent_registry_1.AgentRegistry.getAgentDisplayName(agent.id)}:**\n•${response}`);
                }
                else {
                    // NO FALLBACKS - fail fast with clear error
    ...
    async countAgents() {
        ...
        const agentList = targetAgents.map(agent => {
            const displayName = agent_registry_1.AgentRegistry.getAgentDisplayName(agent.id);
            const description = this.getAgentDescription(agent.id);
            return `• **${displayName}**: ${description}`;
        }).join('\n');
...
Each agent brings unique expertise and can be called upon individually or as part of coordinated multi-agent workflows to handle complex tasks requiring diverse skills.`;
    ...
    async extractAgentsFromResponse(response) {
        // Get all available agents for validation
        const allAgents = await agent_registry_1.AgentRegistry.getAvailableAgents();
