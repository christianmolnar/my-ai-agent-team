...
exports.CommunicationsAgent = void 0;
const child_process_1 = require("child_process"); // I need to import exec for running shell commands
const fs = __importStar(require("fs")); // I need to import fs for file system operations
const path = __importStar(require("path")); // I need to import path for path operations
// Here's my example Communications Agent implementation
class CommunicationsAgent {
    constructor() {
        this.id = 'communications';
        this.name = 'Communications Agent';
        this.description = 'Handles communications, email, meetings, and business writing.';
        // Reference to the central tone and style guide for consistent communication across all agents
        this.toneAndStyleGuidePath = path.join(__dirname, '..', '..', 'shared', 'docs', 'ToneAndStyleGuide.md');
        this.abilities = [
            'Draft Email',
    ...
    async handleTask(task) {
        ...
        if (task.type === 'Convert DOCX to Markdown (Pandoc)') {
            const { filePath, outputFilePath } = task.payload; // I've changed task.details to task.payload
            ...
            const { jobDescriptionPath, resumePath, recipientName, recipientCompany, customNotes } = task.payload;
            ...
            try {
                ...
                const coverLetter = this.generateCoverLetter(jobDescription, resume, recipientName || '', recipientCompany || '', customNotes || '', toneAndStyle || '');
            ...
            const { prompt, referenceImagePath, aspectRatio, resolution, strength } = task.payload;
            if (!prompt) {
     ...
     */
    generateCoverLetter(jobDescription, resume, recipientName, company, customNotes, styleGuide) {
     ...
     */
    extractKeyResponsibilities(jobDescription) {
     ...
     */ async generateImageWithOpenAI({ prompt, referenceImagePath, aspectRatio, resolution, strength }) {
        ...
        const { AI_CONFIG } = await Promise.resolve().then(() => __importStar(require('./ai_config')));
     ...
     */
    async executeCommunicationTask(payload) {
        try {
     ...
     */
    async performCommunicationWork(userRequest, payload) {
        try {
...
Communication strategy provides clear, targeted messaging for effective stakeholder engagement.`;
     ...
     */
    createCommunicationStrategy(request, payload) {
...
COMMUNICATION METHODOLOGY: Strategic messaging with audience-centric approach`;
     ...
     */
    isDocumentCreationRequest(userRequest) {
     ...
     */
    async generateActualDocument(userRequest, payload) {
        try {
            // Use UniversalAIClient like other agents
            const { UniversalAIClient } = await Promise.resolve().then(() => __importStar(require('../lib/universal-ai-client')));
...
Length: ~${documentContent.length} characters`;
     ...
     */
    createBasicDocument(userRequest, payload) {
        return `✅ DOCUMENT COMPLETED: ${userRequest}
...
This document has been created to address your specific request with structured, professional content.`;
     ...
     */
    extractDocumentTitle(userRequest) {
     ...
     */
    generateBasicContent(userRequest) {
        const requestLower = userRequest.toLowerCase();
...
exports.CommunicationsAgent = CommunicationsAgent;
